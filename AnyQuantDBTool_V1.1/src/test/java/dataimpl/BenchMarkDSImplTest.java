...
public class BenchMarkDSImplTest {
	...
	public void testGetBenchMesStringMyDate() {
		...
		MyDate date = DateCalculator.getAnotherDay(-4);
		String code = "000300";
		String nameString = "沪深300";
	...
	public void testGetAllBenchMes() {
		...
		System.out.print("latest date : "+pos.get(0).getDate());
	}


	@Test
	public void updateAllBenchMesFromDate(){
		MyDate start = DateCalculator.getAnotherDay(-5);
		MyDate end = DateCalculator.getToDay();
		String[] codes = {"399001","000001"};
		ArrayList<BenchMarkPO> content = new ArrayList<>();
		for( String code : codes){
			List<BenchMarkPO> temp =  dataService.getBenchMes(code,start,end);
			content.addAll(temp);
		}
