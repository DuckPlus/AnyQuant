...
public class MyFreeChart {
	 ...
	 * @param panel
	 */
	public static void kline_deal_day(List<OHLC_VO> datas_k,List<DealVO> datas_deal,Element config,JPanel panel){
...
//        candlestickRender.setAutoWidthGap(0.001);
      ...
      //设置股票下跌的K线图颜色 
		 ...
		 * 生成x轴,y轴
		 */
        xAxis=createX_Axis(startDate, endDate_plus1,7);
	 ...
	 * @param panel
	 */
	public static void kline_deal_week(List<OHLC_VO> datas_k,List<DealVO> datas_deal,Element config,JPanel panel){
		XYPlot plot_k,plot_deal;//画图区域对象
		MyDate startDate,endDate_plus1;
		MyDate date_temp;
		//设置K线图的画图器，必须申明为final，后面要在匿名内部类里面用到
...
//        candlestickRender.setAutoWidthGap(0.001);
      ...
      //设置股票下跌的K线图颜色 
		 ...
		 * 生成x轴,y轴
		 */
        xAxis=createX_Axis(startDate, endDate_plus1,35);
        yAxis_k=createY_Axis(minValue_k,maxValue_k,10);
		yAxis_deal = createY_Axis(minValue_deal, maxValue_deal, 4);
		// 设置k线图画图区域对象
		plot_k = new XYPlot(seriesCollection_k, xAxis, yAxis_k, candlestickRender);
	 ...
	 * @param panel
	 */
	public static void kline_deal_month(List<OHLC_VO> datas_k,List<DealVO> datas_deal,Element config,JPanel panel){
		XYPlot plot_k,plot_deal;//画图区域对象
		MyDate startDate,endDate_plus1;
		MyDate date_temp;
		//设置K线图的画图器，必须申明为final，后面要在匿名内部类里面用到
...
//        candlestickRender.setAutoWidthGap(0.001);
      ...
      //设置股票下跌的K线图颜色 
		 ...
		 * 生成x轴,y轴
		 */
        xAxis=createX_Axis(startDate, endDate_plus1,150);
        yAxis_k=createY_Axis(minValue_k,maxValue_k,10);
		yAxis_deal = createY_Axis(minValue_deal, maxValue_deal, 4);
		// 设置k线图画图区域对象
		plot_k = new XYPlot(seriesCollection_k, xAxis, yAxis_k, candlestickRender);
	 ...
	 * @param start
	 * @param end
	 * @param day 定义几天画一个横坐标刻度
	 * @return
	 */
	private static DateAxis createX_Axis(MyDate start,MyDate end,int day){
...
//        xAxis.setTimeline(SegmentedTimeline.newMondayThroughFridayTimeline());
      ...
      //设置时间刻度的间隔，一般以周为单位
        xAxis.setTickUnit(new DateTickUnit(DateTickUnit.DAY,day));
        //设置显示时间的格式
